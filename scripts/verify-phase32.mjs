...
async function runPhase32Verification() {
  ...
  try {
    ...
    const sampleDocs = [
      {
        ...
        bodyText:
          'JavaScript is a programming language used for client-side and server-side web development.',
      },
      {
        ...
        bodyText:
          'Hypertext Transfer Protocol transmits web resources and defines client-server communication.',
      },
      {
        ...
        bodyText:
          'Search engines crawl web pages, construct inverted indices, and calculate BM25 relevance.',
      },
      {
    ...
    assert(
      activeIndex !== null && activeIndex.getStats().totalDocuments === sampleDocs.length,
      'Active index loaded with all documents',
    );
    ...
    assert(
      desktopRes.body.includes('id="search-input"') &&
        desktopRes.body.includes('id="search-submit-btn"'),
      'Desktop browser UI renders search input and submit button',
    );
    assert(
      desktopRes.body.includes('id="skip-to-search"') &&
        desktopRes.body.includes('id="a11y-announcer"'),
      'Desktop browser receives full accessibility and skip-link landmarks',
    );
    ...
    assert(
      mobileRes.body.includes(
        '<meta name="viewport" content="width=device-width, initial-scale=1.0">',
      ),
      'Mobile viewport meta tag is present',
    );
    assert(
      mobileRes.body.includes('@media (max-width: 640px)') &&
        mobileRes.body.includes('@media (max-width: 380px)'),
      'Mobile responsive CSS breakpoints are delivered',
    );
    ...
    assert(
      netRes1.statusCode === 200 && netRes1.headers['x-cache'] === 'MISS',
      'First query execution produces X-Cache: MISS',
    );
    const netRes2 = await requestHttp(apiAddr.port, '/api/v1/search?q=javascript');
    assert(
      netRes2.statusCode === 200 && netRes2.headers['x-cache'] === 'HIT',
      'Second identical query executes instantly from LRU cache (X-Cache: HIT)',
    );

    // 4. Empty Queries
    const emptyRes = await requestHttp(apiAddr.port, '/api/v1/search?q=');
    assert(
      emptyRes.statusCode === 200 && emptyRes.json?.results?.length === 0,
      'Empty query returns 200 OK with 0 hits',
    );
    assert(emptyRes.json?.meta?.totalHits === 0, 'Empty query metadata reports 0 total hits');

    // 5. Long Queries
    const longQuery =
      'comprehensive ECMAScript reference guide and modern web development standards';
    const longRes = await requestHttp(
      apiAddr.port,
      `/api/v1/search?q=${encodeURIComponent(longQuery)}`,
    );
    assert(
      longRes.statusCode === 200 && longRes.json?.results?.length > 0,
      'Long multi-word query executes accurately',
    );

    const oversizedQuery = 'x'.repeat(250);
    const oversizedRes = await requestHttp(
      apiAddr.port,
      `/api/v1/search?q=${encodeURIComponent(oversizedQuery)}`,
    );
    assert(
      oversizedRes.statusCode === 400 && oversizedRes.json?.error?.code === 'QUERY_TOO_LONG',
      'Oversized query (>200 chars) is safely rejected with HTTP 400 QUERY_TOO_LONG',
    );

    // 6. No-Result Queries
    const noResultRes = await requestHttp(
      apiAddr.port,
      '/api/v1/search?q=nonexistentquantumsuperstringxyz',
    );
    assert(
      noResultRes.statusCode === 200 && noResultRes.json?.results?.length === 0,
      'No-result query returns 0 hits safely',
    );
    assert(
      noResultRes.json?.meta?.totalHits === 0,
      'No-result query provides zero-count metadata for empty-state UI',
    );

    // 7. Common Searches
    const commonRes = await requestHttp(apiAddr.port, '/api/v1/search?q=http+protocol');
    assert(
      commonRes.statusCode === 200 && commonRes.json?.results?.length > 0,
      'Common search query executes successfully',
    );
    assert(
      commonRes.json?.results[0]?.title?.includes('HTTP'),
      'Top ranked result matches search intent',
    );
    ...
    assert(
      page1Res.statusCode === 200 && page2Res.statusCode === 200,
      'Multi-page pagination endpoints return HTTP 200',
    );
    assert(
      page1Res.json?.pagination?.page === 1 && page2Res.json?.pagination?.page === 2,
      'Pagination metadata tracks page numbers correctly',
    );
    assert(
      page1Res.json?.pagination?.hasNextPage === true &&
        page2Res.json?.pagination?.hasPrevPage === true,
      'Pagination flags hasNextPage and hasPrevPage correctly',
    );

    // 9. External Result Links
    assert(
      desktopRes.body.includes('rel="noopener noreferrer"'),
      'Result links enforce rel="noopener noreferrer" protection',
    );

    // 10. API Failure Behavior
    const invalidParamRes = await requestHttp(apiAddr.port, '/api/v1/search?page=-10');
    assert(
      invalidParamRes.statusCode === 400 &&
        invalidParamRes.json?.error?.code === 'INVALID_PAGE_NUMBER',
      'API failure on invalid parameters returns structured error payload',
    );

    const notFoundRes = await requestHttp(apiAddr.port, '/api/v1/invalid-route');
    assert(
      notFoundRes.statusCode === 404 &&
        (notFoundRes.json?.error?.code === 'NOT_FOUND_ERROR' ||
          notFoundRes.json?.error?.code === 'NOT_FOUND'),
      'Unmatched API routes return clean 404 JSON',
    );
  ...
  if (passedChecks === totalChecks) {
    console.log(
      'Phase 32 is FULLY VERIFIED and READY for Phase 33 (Final Security & Release Audit).\n',
    );
  } else {
    process.exit(1);
  }
